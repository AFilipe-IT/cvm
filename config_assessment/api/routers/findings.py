...
def list_findings(
    ...
    With `scan_id`, findings from THAT assessment alone. Without it the list
    spans the estate, which is right for triage and wrong immediately after
    running a scan: someone who has just assessed one file is asking what this
    run found, and answering with the whole history buries it among findings
    from configurations they did not touch.
    """
    ...
    if scan_id is not None:
        # An unknown scan is a 404, not an empty list: "this assessment found
        # nothing" and "there is no such assessment" are the same not_assessed
        # /clean confusion the rest of this codebase refuses to make, and here
        # the API can be unambiguous for free.
        one = db.get_scan_result(scan_id)
        if one is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail={"error": {"code": "not_found",
